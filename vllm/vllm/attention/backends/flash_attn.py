...
class FlashAttentionMetadata(AttentionMetadata):
    ...
    # Hybrid embed-only fast path metadata (prefill-only)
    # List of request_ids for prefill sequences in-batch order (python only).
    prefill_request_ids: Optional[List[str]] = None
    # Boolean mask of length=num_prefills indicating embed-only prefill
    # sequences that should not write KV to paged cache.
    no_kv_cache_prefill_seq_mask: Optional[torch.Tensor] = None
    # Ephemeral KV mode for embed-only: 'scratch' or 'recompute'.
    embed_ephemeral_kv_mode: Optional[str] = None
...
class FlashAttentionImpl(AttentionImpl):
    ...
    def forward(
        ...
        if kv_cache.numel() > 0:
            ...
            # Defer KV cache write until after prefill-vs-decode slicing so we
            # can selectively skip embed-only tokens.
            if fp8_attention:
        ...
        # Perform paged-KV write for prefill tokens only, with optional mask
        # to skip embed-only tokens in mixed batches.
        if (kv_cache.numel() > 0) and (attn_type != AttentionType.ENCODER) \
                and (key is not None) and (value is not None) \
                and (num_prefill_kv_tokens > 0):
            kv_key = key[:num_prefill_kv_tokens]
            kv_value = value[:num_prefill_kv_tokens]
            ...
            token_mask = None
            prefill_meta_for_mask = attn_metadata.prefill_metadata
            if (prefill_meta_for_mask is not None
                    and getattr(prefill_meta_for_mask,
                                "no_kv_cache_prefill_seq_mask", None)
                    is not None and
                    prefill_meta_for_mask.no_kv_cache_prefill_seq_mask
                    is not None and
                    prefill_meta_for_mask.no_kv_cache_prefill_seq_mask
                    .numel() > 0):
                qs = prefill_meta_for_mask.query_start_loc
                num_prefills = prefill_meta_for_mask.num_prefills
                token_mask = torch.empty(num_prefill_kv_tokens,
                                         dtype=torch.bool,
                                         device=query.device)
                for i in range(num_prefills):
                    start = int(qs[i].item())
                    end = int(qs[i + 1].item())
                    allow_write = not bool(
                        prefill_meta_for_mask
                        .no_kv_cache_prefill_seq_mask[i].item())
                    if start < end:
                        token_mask[start:end] = allow_write
            ...
            if token_mask is not None:
                if token_mask.any():
                    torch.ops._C_cache_ops.reshape_and_cache_flash(
                        ...
                        kv_cache[0],
                        kv_cache[1],
                        updated_slot_mapping[:num_prefill_kv_tokens]
                        [token_mask].flatten(),  # type: ignore[union-attr]
                        kv_cache_dtype,
                        layer._k_scale,
                        layer._v_scale,
                    )
                ...
                torch.ops._C_cache_ops.reshape_and_cache_flash(
                    ...
                    kv_cache[1],
                    updated_slot_mapping[:num_prefill_kv_tokens]
                    .flatten(),  # type: ignore[union-attr]
                    kv_cache_dtype,
                    layer._k_scale,
                    layer._v_scale,
                )
