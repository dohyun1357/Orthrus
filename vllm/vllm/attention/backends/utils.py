...
class CommonMetadataBuilder(AttentionMetadataBuilder[TAttentionMetadata]):
    ...
    def prepare(self):
        ...
        # Hybrid embeddings: record per-prefill ids and no-kv mask
        self.prefill_request_ids: List[str] = []
        self.no_kv_cache_prefill_seq_mask: List[bool] = []
    ...
    def _add_seq_group(
        ...
        for (seq_id, token_len, seq_len, curr_seq_len, query_len, context_len,
            ...
            if is_prompt:
                ...
                # Hybrid embeddings fast path bookkeeping
                # InterDataForSeqGroup is extended in the GPU builder
                # to include 'no_kv_cache_embed' flag and request_id.
                request_id = getattr(inter_data, "request_id", None)
                if request_id is None:
                    request_id = ""
                self.prefill_request_ids.append(request_id)
                self.no_kv_cache_prefill_seq_mask.append(
                    bool(getattr(inter_data, "no_kv_cache_embed", False)))
            else:
                assert query_len == 1, (
    ...
    def build(self, seq_lens: List[int], query_lens: List[int],
        ...
        # Attach hybrid embed metadata (python + device tensors)
        # Attach hybrid embed metadata for any backend that supports fields.
        if hasattr(metadata, "prefill_request_ids"):
            metadata.prefill_request_ids = self.prefill_request_ids
        if hasattr(metadata, "no_kv_cache_prefill_seq_mask") and \
                self.no_kv_cache_prefill_seq_mask:
            setattr(
                metadata,
                "no_kv_cache_prefill_seq_mask",
                async_tensor_h2d(
                    self.no_kv_cache_prefill_seq_mask,
                    torch.bool,
                    device,
        ...
        if hasattr(metadata, "embed_ephemeral_kv_mode") and \
                hasattr(self.runner, "vllm_config"):
            mode = self.runner.vllm_config.additional_config.get(
                "embed_ephemeral_kv_mode", "scratch")
            setattr(metadata, "embed_ephemeral_kv_mode", mode)
