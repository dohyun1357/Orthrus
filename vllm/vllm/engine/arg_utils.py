...
class EngineArgs:
    ...
    # Hybrid embeddings fast path (xFormers) flags
    # When True, embed-only requests (HybridPooler, no decoding)
    # take a cacheless prefill path on xFormers backend.
    embed_no_kv_cache: bool = True
    # Ephemeral KV handling for embed-only cacheless prefill.
    # 'scratch' maintains per-layer scratch across chunks; 'recompute' rebuilds per chunk.
    embed_ephemeral_kv_mode: str = "scratch"
    ...
    def add_cli_args(parser: FlexibleArgumentParser) -> FlexibleArgumentParser:
        ...
        # Hybrid embeddings fast path flags (xFormers only)
        parser.add_argument(
            "--embed-no-kv-cache",
            action=argparse.BooleanOptionalAction,
            default=EngineArgs.embed_no_kv_cache,
            help=(
                "For embed-only requests (HybridPooler), avoid writing K/V "
                "to the paged KV cache during prefill on xFormers backend."))
        parser.add_argument(
            "--embed-ephemeral-kv-mode",
            type=str,
            choices=["scratch", "recompute"],
            default=EngineArgs.embed_ephemeral_kv_mode,
            help=(
                "KV handling for embed-only cacheless prefill: 'scratch' "
                "maintains ephemeral per-layer K/V across chunks; 'recompute' "
                "rebuilds prefix per chunk."))
    ...
    def create_engine_config(
        ...
        # Inject hybrid embed flags into additional_config for workers/backends.
        # Do this after constructing config so it is visible downstream.
        config.additional_config.setdefault("embed_no_kv_cache",
                                           self.embed_no_kv_cache)
        config.additional_config.setdefault("embed_ephemeral_kv_mode",
                                           self.embed_ephemeral_kv_mode)
