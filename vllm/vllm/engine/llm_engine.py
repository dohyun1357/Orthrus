...
from vllm.sequence import (CompletionSequenceGroupOutput, ExecuteModelRequest, ParallelSampleSequenceGroup, PoolerOutput,
...
class LLMEngine:
    ...
    def _process_hybrid_pooling_sequence_group_outputs(
        self,
        seq_group: SequenceGroup,
        outputs: List[PoolingSequenceGroupOutput] | List[CompletionSequenceGroupOutput],
    ) -> bool:
        if isinstance(outputs[0], PoolingSequenceGroupOutput) and outputs[0].data is not None:
            return True
        return False
    ...
    @staticmethod
    def _process_hybrid_sequence_group_outputs(
        seq_group: SequenceGroup,
    ...
    def _process_model_outputs(self,
        ...
        if has_multiple_outputs:
            assert self.scheduler_config.is_multi_step or \
                     self.speculative_config or self.model_config.runner_type == "hybrid"
            ...
            elif self.model_config.runner_type == "hybrid":

                pooler_output = next(o for o in outputs if isinstance(o, PoolerOutput))
                sampler_output = next(o for o in outputs if isinstance(o, SamplerOutput))

                # build iterators
                pool_iter = iter(pooler_output.outputs)
                sampl_iter = iter(sampler_output.outputs)

                # now distribute them in the same order as seq_group_metadata_list
                outputs_by_sequence_group = []
                for sg_meta in seq_group_metadata_list:
                    if sg_meta.pooling_params is not None:
                        # this seq group asked for pooling
                        outputs_by_sequence_group.append([ next(pool_iter) ])
                    elif sg_meta.sampling_params is not None:
                        # this seq group asked for sampling
                        outputs_by_sequence_group.append([ next(sampl_iter) ])
                    else:
                        # no-op sequences (unlikely)
        ...
        for i in indices:
            ...
            if self.model_config.runner_type == "pooling":
                ...
                if seq_group_meta.pooling_params is not None:
                    if self._process_hybrid_pooling_sequence_group_outputs(
                            seq_group, output):
                        self._process_hybrid_sequence_group_outputs(
                            seq_group, seq_group_meta, output)
                else:
                    # this was a generate/sampling request
    ...
    def _advance_to_next_step(
            self, output: SamplerOutput | PoolingSequenceGroupOutput,
            seq_group_metadata_list: List[SequenceGroupMetadata],
        ...
        for seq_group_metadata, sequence_group_outputs, scheduled_seq_group in \
            ...
            seq_group = scheduled_seq_group.seq_group
            if seq_group_metadata.do_sample and isinstance(sequence_group_outputs, PoolingSequenceGroupOutput):
                for seq in seq_group.get_seqs():
                    seq.status = SequenceStatus.FINISHED_STOPPED
