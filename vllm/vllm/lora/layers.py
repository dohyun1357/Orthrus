...
class LogitsProcessorWithLoRA(BaseLayerWithLoRA):
    ...
    def _get_logits(
        ...
        lora_row_indices = getattr(forward_context, 'lora_row_indices', None)

        if lora_row_indices is None or lora_row_indices.numel() == 0:
        ...
        lora_hidden_states = hidden_states.index_select(0, lora_row_indices)

        lora_a = self.lora_a_stacked[0].squeeze(0)
        lora_b = self.lora_b_stacked[0].squeeze(0)
        
        lora_a_output = F.linear(lora_hidden_states, lora_a)
        lora_logits_delta = F.linear(lora_a_output, lora_b)

        logits.index_add_(0, lora_row_indices, lora_logits_delta)
