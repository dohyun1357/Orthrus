...
class LLM:
    ...
    def generate(
        ...
        if runner_type not in ["generate", "transcription", "hybrid"]:
            messages = [
    ...
    def hybrid_embed(
        ...
        if runner_type != "hybrid":
            messages = ["LLM.hybrid_embed() is only supported for task = hybrid"]

            supported_runner_types = self.llm_engine.model_config \
                .supported_runner_types
            if "generate" in supported_runner_types:
                messages.append(
                    "Your model supports the 'hybrid' runner, but is "
                    f"currently initialized for the '{runner_type}' runner. "
                    "Please initialize vLLM using `--task hybrid`")
