...
def _unwrap_seq_group(x):
    """Return the SequenceGroup object regardless of wrapper shape."""
    return getattr(x, "seq_group", x)

def _class_of_seq_group(sg) -> str:
    # 1) explicit tag
    rtype = getattr(sg, "request_type", None)
    if rtype is None:
        rtype = getattr(getattr(sg, "metadata", None), "request_type", None)
    if rtype in ("embed", "gen"):
        return rtype

    # 2) task marker
    task = getattr(sg, "task", None)
    if task is None:
        task = getattr(getattr(sg, "metadata", None), "task", None)
    if task == "embed":
        return "embed"

    # 3) prefill-only / no decoding configured
    if getattr(sg, "is_prefill_only", False):
        return "embed"
    sp = getattr(sg, "sampling_params", None)
    max_new = getattr(sp, "max_tokens", None)
    if max_new is None or int(max_new) == 0:
        return "embed"

    try:
        rem_decode = int(getattr(sg, "num_generated_tokens", 0))
        if rem_decode <= 0:
            first = getattr(sg, "first_seq", None)
            if first is not None:
                try:
                    seq_len = int(first.data.get_len())
                except Exception:
                    seq_len = int(getattr(first, "get_len", lambda: 0)())
                prompt_len = int(getattr(first.data, "prompt_len", 0))
                gen_so_far = max(0, seq_len - prompt_len)
                if gen_so_far <= 0:
                    return "embed"
    except Exception:
        pass
...
class Scheduler:
    ...
    def __init__(
        ...
        # Optional per-step queue logging (prototype)
        self._log_file = None
        log_base = os.getenv("VLLM_SCHEDULER_LOG_FILE")
        if log_base:
            try:
                log_path = f"{log_base}{self.scheduler_config.runner_type}.csv"
                self._log_file = open(log_path, "w")
                # Write header for convenience
                self._log_file.write("ts,embed_waiting,gen_waiting\n")
                self._log_file.flush()
            except Exception as e:
                logger.warning("Failed to open scheduler log file: %s", e)

        # Adaptive limiter to reduce swap/preempt thrash from prefills.
        # Start permissive; back off on preemption; slowly relax when stable.
        self._prefill_slots_target: int = max(1, self.scheduler_config.max_num_partial_prefills)
        self._prefill_slots_min: int = 1
        self._prefill_slots_max: int = max(1, self.scheduler_config.max_num_partial_prefills)
        self._turn_class: str = "gen"
    ...
    def _split_waiting_by_class(self) -> tuple[deque, deque]:
        waitingE, waitingG = deque(), deque()
        for item in self.waiting:
            sg = _unwrap_seq_group(item)
            cls = _class_of_seq_group(sg)
            if cls == "embed":
                waitingE.append(item)  # keep the original object to preserve any wrapper fields
            else:
                waitingG.append(item)
        return waitingE, waitingG
    
    def _schedule_split(self) -> SchedulerOutputs:
        budget = SchedulingBudget(
            token_budget=self.scheduler_config.max_num_batched_tokens,
            max_num_seqs=self.scheduler_config.max_num_seqs,
        )
        ...
        ppm_all = PartialPrefillMetadata.from_queues(
            running=self.running,
            waiting=self.waiting,
            scheduler_config=self.scheduler_config,
        )
        running_scheduled = self._schedule_running(
            ...
            curr_loras,
            enable_chunking=True,
            partial_prefill_metadata=ppm_all,
        )
        ...
        waitingE, waitingG = self._split_waiting_by_class()

        prefills = SchedulerPrefillOutputs.create_empty()
        if budget.remaining_token_budget() > 0 and (waitingE or waitingG):
            merged_waiting: Deque[SequenceGroup] = deque()
            wE = len(waitingE)
            wG = len(waitingG)
            if wE == 0:
                merged_waiting = waitingG
            elif wG == 0:
                merged_waiting = waitingE
            else:
                prioE = 1.0
                prioG = 4.0
                accE = 0.0
                ...
                qE = float(wE) * prioE
                qG = float(wG) * prioG
                denom = qE + qG if (qE + qG) > 0 else 1.0
                while waitingE or waitingG:
                    if not waitingE:
                        merged_waiting.extend(waitingG)
                        break
                    if not waitingG:
                        merged_waiting.extend(waitingE)
                        break
                    accE += qE
                    accG += qG
                    if accE >= accG:
                        merged_waiting.append(waitingE.popleft())
                        accE -= denom
                    else:
                        merged_waiting.append(waitingG.popleft())
                        accG -= denom

            orig_waiting = self.waiting
            try:
                self.waiting = merged_waiting
                ppm_merged = PartialPrefillMetadata.from_queues(
                    running=self.running,
                    waiting=self.waiting,
                    scheduler_config=self.scheduler_config,
                )
                preempt_cnt = len(running_scheduled.preempted) + len(running_scheduled.swapped_out)
                if preempt_cnt > 0:
                    self._prefill_slots_target = max(
                        self._prefill_slots_min,
                        int(max(self._prefill_slots_min, self._prefill_slots_target) * 0.7),
                    )
                else:
                    if self._prefill_slots_target < self._prefill_slots_max:
                        self._prefill_slots_target += 1
                ppm_merged.schedulable_prefills = max(
                    self._prefill_slots_min,
                    min(ppm_merged.schedulable_prefills, self._prefill_slots_target),
                )
                prefills = self._schedule_prefills(
                    budget,
                    curr_loras,
                    enable_chunking=True,
                    partial_prefill_metadata=ppm_merged,
                )
                leftover = self.waiting
            finally:
                self.waiting = orig_waiting
            self.waiting = deque(leftover)
        ...
        all_prefills = len(scheduled_seq_groups) == num_prefill_groups
        num_lookahead_slots = 0 if (all_prefills and not self.scheduler_config.is_multi_step) \
            else running_scheduled.num_lookahead_slots
    ...
    def _schedule_turn_split(self) -> SchedulerOutputs:
        """Alternate batches of generation and embedding requests."""
        ...
        original_running = deque(self.running)
        original_swapped = deque(self.swapped)

        def queue_has(queue: Iterable[SequenceGroup], cls: str) -> bool:
            for item in queue:
                if _class_of_seq_group(_unwrap_seq_group(item)) == cls:
                    return True
            return False

        running_has_gen = queue_has(original_running, "gen")
        running_has_embed = queue_has(original_running, "embed")
        swapped_has_gen = queue_has(original_swapped, "gen")
        swapped_has_embed = queue_has(original_swapped, "embed")

        def has_class_pre(cls: str) -> bool:
            if cls == "gen":
                return bool(waitingG) or running_has_gen or swapped_has_gen
            return bool(waitingE) or running_has_embed or swapped_has_embed

        active_turn = self._turn_class
        other_turn = "embed" if active_turn == "gen" else "gen"
        if not has_class_pre(active_turn) and has_class_pre(other_turn):
            active_turn = other_turn
            self._turn_class = active_turn

        running_selected: Deque[SequenceGroup] = deque()
        running_others: Deque[SequenceGroup] = deque()
        for seq_group in original_running:
            if _class_of_seq_group(seq_group) == active_turn:
                running_selected.append(seq_group)
            else:
                running_others.append(seq_group)
        self.running = running_selected

        swapped_selected: Deque[SequenceGroup] = deque()
        swapped_others: Deque[SequenceGroup] = deque()
        for seq_group in original_swapped:
            if _class_of_seq_group(seq_group) == active_turn:
                swapped_selected.append(seq_group)
            else:
                swapped_others.append(seq_group)
        self.swapped = swapped_selected
        ...
        prefills = SchedulerPrefillOutputs.create_empty()
        if active_turn == "gen":
            selected_waiting: Deque[SequenceGroup] = deque(waitingG)
            other_waiting = waitingE
        else:
            selected_waiting = deque(waitingE)
            other_waiting = waitingG
        leftover_selected: Deque[SequenceGroup] = deque(selected_waiting)

        if budget.remaining_token_budget() > 0 and selected_waiting:
            orig_waiting = self.waiting
            try:
                self.waiting = selected_waiting
                ppm_selected = PartialPrefillMetadata.from_queues(
                    running=self.running,
                    waiting=self.waiting,
                    scheduler_config=self.scheduler_config,
                )
                preempt_cnt = (len(running_scheduled.preempted) +
                               len(running_scheduled.swapped_out))
                if preempt_cnt > 0:
                    self._prefill_slots_target = max(
                        self._prefill_slots_min,
                    ...
                    if self._prefill_slots_target < self._prefill_slots_max:
                        self._prefill_slots_target += 1
                ppm_selected.schedulable_prefills = max(
                    self._prefill_slots_min,
                    min(ppm_selected.schedulable_prefills, self._prefill_slots_target),
                )
                prefills = self._schedule_prefills(
                    budget,
                    curr_loras,
                    enable_chunking=True,
                    partial_prefill_metadata=ppm_selected,
                )
                leftover_selected = deque(self.waiting)
            finally:
                self.waiting = orig_waiting

        new_waiting: Deque[SequenceGroup] = deque()
        new_waiting.extend(leftover_selected)
        new_waiting.extend(other_waiting)
        ...
        self.swapped.extend(running_scheduled.swapped_out)
        self.swapped.extend(swapped_others)

        self.running.extend(running_others)
        ...
        all_prefills = len(scheduled_seq_groups) == num_prefill_groups
        num_lookahead_slots = (0 if (all_prefills and
        ...
        def has_class_post(cls: str) -> bool:
            return (queue_has(self.waiting, cls) or queue_has(self.running, cls)
                    or queue_has(self.swapped, cls))

        if not has_class_post(active_turn):
            other_cls = "embed" if active_turn == "gen" else "gen"
            if has_class_post(other_cls):
                self._turn_class = other_cls
            else:
                self._turn_class = "gen"
        else:
            self._turn_class = active_turn
    ...
    def _schedule(self) -> SchedulerOutputs:
        """Schedule queued requests."""
        if self.scheduler_config.chunked_prefill_enabled:
            if os.getenv("ORTHRUS_SCHEDULER") == "turn_split":
                return self._schedule_turn_split()
            elif os.getenv("ORTHRUS_SCHEDULER") == "split":
                return self._schedule_split()
            else:
